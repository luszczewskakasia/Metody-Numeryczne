...
def compare_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if x1.shape != y1.shape or x2.shape != y2.shape or min(x1.shape) or min(x2.shape):
        return None

    fig, ax = plt.subplots()
    ax.plot(x1, y1, '-b', widthline=4)
    ax.plot(x2, y2, '-r', widthline=2)
    plt.plot(label1=input(), label2=input(), title=input(),  xlabel=input(), ylabel=input())
    plt.plot(legend=input())
...
def parallel_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    """Funkcja służąca do stworzenia dwóch wykresów typu plot w konwencji subplot wertykalnie lub horyzontalnie. 
...
def log_plot(x:np.ndarray,y:np.ndarray,xlabel:np.ndarray,ylabel:str,title:str,log_axis:str):
    if x.shape != y.shape or  min(x.shape):
        return None
    
    fig, ax = plt.semilogy(log_axis(x, y))
    plt.plot(x=input(), y =input(), xlabel=input(), ylabel=input(), title=input())
    ...
    if log_axis == "x":
        ...
        ax.set_xs("log")
        ax.set_ys("log") #czy to trzeba robić? bo ja tego nei miałam i przeszło testy xd
    else:
        return None
